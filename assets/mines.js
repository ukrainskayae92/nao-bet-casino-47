...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-e18600ab";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-e18600ab">'+'<span class="tile-face-e18600ab tile-front-e18600ab"></span>'+'<span class="tile-face-e18600ab tile-back-e18600ab"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-e18600ab")){return;}
var back=tile.querySelector(".tile-back-e18600ab");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-e18600ab");if(isMine){tile.classList.add("boom-e18600ab");}
if(dim){tile.classList.add("dimmed-e18600ab");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-e18600ab")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-e18600ab");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-e18600ab")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-e18600ab");void panelEl.offsetWidth;panelEl.classList.add("jolt-e18600ab");}
endRound();}
